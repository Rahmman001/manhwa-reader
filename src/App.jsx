...
function HomeView({ series, openSeries, openAdmin, genres, selectedGenre, setSelectedGenre }) {
  return <section><div className="relative mb-10 overflow-hidden rounded-xl bg-gradient-to-r from-[#3b0b0f] to-[#232323] p-8 md:p-14"><div className="relative z-10 max-w-xl"><p className="mb-3 text-xs font-bold uppercase tracking-[0.3em] text-[#E50914]">Personal library</p><h1 className="mb-4 text-4xl font-black md:text-6xl">Read your way.</h1><p className="mb-6 text-gray-300">Upload a chapter PDF, convert it in your browser, and read it as a smooth vertical webtoon.</p><button onClick={openAdmin} className="rounded-md bg-[#E50914] px-5 py-3 font-bold hover:bg-red-700">Upload a chapter</button></div></div><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-bold">Your series</h2><p className="mt-1 text-sm text-gray-500">Search by title or genre from the navbar.</p></div><span className="text-sm text-gray-500">{series.length} series</span></div>{genres.length > 0 && <div className="mb-6 flex flex-wrap gap-2"><button onClick={() => setSelectedGenre('')} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${selectedGenre === '' ? 'bg-[#E50914] text-white' : 'bg-[#232323] text-gray-400 hover:bg-[#303030] hover:text-white'}`}>All genres</button>{genres.map((genre) => <button key={genre} onClick={() => setSelectedGenre(selectedGenre === genre ? '' : genre)} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${selectedGenre === genre ? 'bg-[#E50914] text-white' : 'bg-[#232323] text-gray-400 hover:bg-[#303030] hover:text-white'}`}>{genre}</button>)}</div>}{series.length === 0 ? <EmptyState text={selectedGenre ? `No series found in ${selectedGenre}.` : 'No series yet. Create one from Admin.'} /> : <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{series.map((item) => <SeriesCard key={item.id} series={item} onClick={() => openSeries(item)} />)}</div>}</section>
}
